...
for x in range(x_min,x_max, size_degree):
    ...
    for y in range(y_min,y_max, size_degree):
        if cpt>=cpt_min:
            ...
            print(cur_bbox)
            xml_query='<osm-script output="xml" timeout="25"><union><query type="node"><has-kv k="waterway"/><bbox-query s="'+cur_y_min+'" w="'+cur_x_min+'" n="'+cur_y_max+'" e="'+cur_x_max+'"/></query><query type="way"><has-kv k="waterway"/><bbox-query s="'+cur_y_min+'" w="'+cur_x_min+'" n="'+cur_y_max+'" e="'+cur_x_max+'"/></query><query type="relation"><has-kv k="waterway"/><bbox-query s="'+cur_y_min+'" w="'+cur_x_min+'" n="'+cur_y_max+'" e="'+cur_x_max+'"/></query></union><union><item/><recurse type="down"/></union><print mode="body"/></osm-script>'
            #print(xml_query)
